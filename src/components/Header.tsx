...
export default function Header() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "nameVisibility",
      handleNameVisibility as EventListener
    );
    return () => {
  ...
  return (
    <header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 border-b ${
        isOpaque
          ? "bg-background border-accent-subtle"
      ...
      <div className="max-w-5xl mx-auto px-8 sm:px-12 lg:px-16">
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center overflow-hidden">
            ...
            >
              {isHeroVisible ? (
                <span className="px-2 py-1 text-foreground transition-colors duration-300">
                  Adam Archer
                </span>
              ) : (
                <a
                  href="#top"
                  className="px-2 py-1 rounded-md text-foreground hover:text-foreground focus:outline-none focus:ring-2 focus:ring-primary transition-colors duration-300"
                >
                  Adam Archer
                </a>
              )}
          ...
          <div className="flex items-center space-x-4">
            <nav className="hidden sm:flex space-x-4">
              <a
                href="#about"
                className="px-2 py-1 rounded-md text-muted-foreground hover:text-foreground focus:outline-none focus:ring-2 focus:ring-primary transition-colors duration-300"
              >
                About
              </a>
              <a
